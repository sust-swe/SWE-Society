    ...
    return (
        <Layout>
            <Flex direction="row" >
                <Box padding="10">
                    {loadPromise ? (
                        <Image boxSize="2xl" src={String(event.image[0])} />
                    ) : (
                            <React.Fragment />
                        )}
                </Box>
                <Flex direction="column" justifyContent="center">
                    <Text fontFamily="heading" marginTop="10" marginLeft="10" fontSize="4xl" fontWeight="bold">{event.title}</Text>
                    <Flex direction="row">
                        <Icon marginLeft="10" marginBottom="5" size="5xl" as={IoLocation}></Icon>
                        <Text marginLeft="2">{event.location}</Text>
                    </Flex>
                    <Flex direction="row">
                        <Icon marginLeft="10" size="5xl" as={FaCalendarAlt}></Icon>
                        <Text marginLeft="2">{new Date(event.event_date).getUTCDay()}/{new Date(event.event_date).getUTCMonth()}/{new Date(event.event_date).getUTCFullYear()}</Text>
                    </Flex>
